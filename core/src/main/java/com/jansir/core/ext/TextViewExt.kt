...
fun TextView.sizeDrawable(width: Int, height: Int, leftDrawable: Int = 0, topDrawable: Int = 0,
    ...
    setCompoundDrawables(
            findDrawable(leftDrawable, 0, this)
                ?.apply { bounds = rect },
            findDrawable(topDrawable, 1, this)
                ?.apply { bounds = rect },
            findDrawable(rightDrawable, 2, this)
                ?.apply { bounds = rect },
            findDrawable(bottomDrawable, 3, this)
                ?.apply { bounds = rect }
    )
    return this
}
...
//    if(drawableRes!=0)return com.jansir.base.ext.drawable(drawableRes)
